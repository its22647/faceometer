...
async function checkFaceVisibility() {
    if (!faceDetector) return true; // Safety fallback
    const predictions = await faceDetector.estimateFaces(videoElement, false);
    return predictions.length > 0; // Return true if at least one face found
}

// --- 4. UI: DRAWER & MODERN POPUP ---
function showProAlert(message, type = "error") {
    const color = type === "warning" ? "#ffcc00" : "#ff3b3b";
    const overlay = document.createElement('div');
    overlay.style = `position: fixed; inset: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(5px); z-index: 20000; display: flex; align-items: center; justify-content: center; padding: 20px; font-family: 'Orbitron', sans-serif;`;
    const box = document.createElement('div');
    box.style = `background: #000; border: 2px solid ${color}; width: 100%; max-width: 400px; padding: 30px; box-shadow: 0 0 30px ${color}44; transition: 0.3s; opacity: 0; transform: scale(0.9);`;
    const content = document.createElement('div');
    content.style = `color: #fff; font-size: 14px; line-height: 1.6; min-height: 50px; font-weight: 700;`;
    const btn = document.createElement('button');
    btn.style = `margin-top: 25px; width: 100%; background: ${color}; border: none; color: #000; padding: 12px; font-weight: 900; cursor: pointer; clip-path: polygon(0 0, 100% 0, 95% 100%, 5% 100%);`;
...
function initProtocolDrawer() {
    ...
    modeSelect.parentElement.appendChild(mainTrigger);
    const grid = document.createElement('div');
    grid.style = "display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 12px; width: 100%; max-height: 0; overflow: hidden; opacity: 0; transition: all 0.5s;";
    modes.forEach((mode, index) => {
        const tile = document.createElement('div');
        tile.className = "mode-tile";
        tile.style = `background: #111; border: 1px solid #333; padding: 15px 5px; border-radius: 12px; cursor: pointer; text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; transform: translateY(20px); transition: 0.3s; transition-delay: ${index * 0.1}s;`;
        ...
        tile.onclick = () => {
            ...
            currentMode = mode; mainTrigger.textContent = mode.name.toUpperCase();
            grid.style.maxHeight = "0"; grid.style.opacity = "0";
        };
        grid.appendChild(tile);
    });
    modeSelect.parentElement.appendChild(grid);
    mainTrigger.onclick = () => { grid.style.maxHeight = grid.style.maxHeight === "500px" ? "0" : "500px"; grid.style.opacity = grid.style.opacity === "1" ? "0" : "1"; document.querySelectorAll('.mode-tile').forEach(t => t.style.transform = grid.style.opacity === "1" ? "translateY(0)" : "translateY(20px)"); };
}
...
function typeEffectOnCanvas(fullText) {
    ...
    function draw() {
        if (wordIndex < wordsArr.length) {
            ...
            let lines = []; let line = ""; let currentWords = currentText.split(" ");
            for(let n=0; n<currentWords.length; n++){
                let testLine = line + currentWords[n] + " ";
                if(context.measureText(testLine).width > canvasWidth * 0.9 && n > 0){ lines.push(line); line = currentWords[n] + " "; } 
                else { line = testLine; }
            }
            lines.push(line);
